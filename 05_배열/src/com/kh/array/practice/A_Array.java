...
//		int sum = score1 + score2 + score3 + score4 + score5;
	...
	public void method2() {
		...
		// 배열 생성
//		iArray1 = new int[]; // 배열의 크기를 지정하지 않으면 error

		iArray1 = new int[5]; // 메모리(Heap)에 5개의 정수형 데이터를 저장할 수 있는 배열을 생성한다.-> 배열의 시작 주소를 참조 변수에 대입한다.
		iArray2 = new int[10]; // 메모리(Heap)에 10개의 정수형 데이터를 저장할 수 있는 배열을 생성한다.-> 배열의 시작 주소를 참조 변수에 대입한다.
		dArray1 = new double[5]; // 메모리(Heap)에 5개의 실수형 데이터를 저장할 수 있는 배열을 생성한다.-> 배열의 시작 주소를 참조 변수에 대입한다.
		dArray2 = new double[10]; // 메모리(Heap)에 10개의 실수형 데이터를 저장할 수 있는 배열을 생성한다.-> 배열의 시작 주소를 참조 변수에 대입한다.
		 ...
		 * 배열 초기화
		 * 
		 * 배열은 따로 초기화하지 않는다면, JVM이 지정한 기본 값으로 배열을 초기화한다. (정수형 : 0, 실수형 : 0.0, 문자형 :
		 * '\u0000', 논리형 : false, 참조형 : null)
		 */
		 ...
		 * 배열을 초기화하는 for 문
		 * 
		 * 배열에서 배열의 길이를 확인하는 방법은 length 필드(변수)를 통해서 제공하는 것이고 문자열에서 문자열의 길이를 확인하는 방법은
		 * length() 메소드를 통해서 제공한다.
		 */
...
//		int[] iArray2; // error
//		iArray2 = {1, 2, 3, 4}; // error 중괄호로만 초기화하는게 안된다
//		iArray2 = new int [] {1, 2, 3, 4}; // error
	 ...
	 * 실습 문제 1
	 * 
	 * 사용자가 입력하는 정수값으로 배열의 길이는 지정하여 배열을 생성하고 생성된 배열의 크기만큼 반복문을 실행하여 랜덤값을 (1 ~ 100)
	 * 배열에 저장하고 출력한다.
	 * 
	 * 예시) 정수값을 입력해 주세요 > 3
	 * 
	 * numbers[0] : 77 numbers[1] : 88 numbers[2] : 6
	 */
	 ...
	 * 실습 문제 2
	 * 
	 * 사용자에게 3명의 키를 입력받아 배열에 저장하고 반복문을 통해 3명의 키의 총합, 평균값을 구하시오.
	 * 
	 * 예시) 키 입력 > 180.0 키 입력 > 177.3 키 입력 > 168.2
	 * 
	 * 총 합 : 525.5 평 균 : 175.2
	 */
