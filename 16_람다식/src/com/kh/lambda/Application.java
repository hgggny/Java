...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 * 		- 추상메서드가 하나만 있는 인터페이스만 람다식으로 대입이 가능하다. - 함수적 인터페이스(Functional Interface)
		 * 	[표현법]
		 * 		([타입 매개변수, ...]) -> {
		 * 			// 실행 코드
		 * 
...
//		new A_Lambda().method1();
		...
		/*
		 * 타겟 타입(Target Type)
		 * 	- 람다식은 매개 변수를 가진 코드 블록이기 때문에 자바의 메소드 선언처럼 보여지지만 자바는 메소드를 단독으로 선언할 수 없고 클래스의 구성 멤버로 선언하기 때문에 람다식은 단순히 메소드를
		 ...
		 *  - 하나의 추상 메서드가 선언된 인터페이스만 타겟이 될 수 있는데 이러한 인터페이스를 함수적 인터페이스라고 한다. 
